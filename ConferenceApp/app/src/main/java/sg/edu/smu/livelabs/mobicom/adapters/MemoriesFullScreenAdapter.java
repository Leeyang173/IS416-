 ...
 */
public class MemoriesFullScreenAdapter extends PagerAdapter {
    private Context context;
    private List<MemoriesItem> images;
    private MemoriesFullScreenPresenter listener;
    public MemoriesFullScreenAdapter(Context context, List<MemoriesItem> images, MemoriesFullScreenPresenter listener){
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        VH.setData(images.get(position), position);
        view.setTag(VH);
    ...
    public class ViewHolder implements View.OnClickListener{
        ...
        public void setData(MemoriesItem photo, int position){
            Picasso.with(context).cancelRequest(image);
            if (photo.image != null && !photo.image.isEmpty()){
                try {
                    Picasso.with(context).load(Util.getPhotoUrlFromId(photo.image, 512))
                            .noFade()
